...
- Authentication → Email Templates → paste the Yarn-branded HTML from
  `supabase/templates/` (Confirm sign up, Magic Link, and the other auth
  emails). Subjects and bodies are generated by
  `node scripts/yarn-auth-email-templates.mjs`; push live with
