...
function InviteLanding({ code }: { code: string }) {
  ...
  const { account, authEnabled, authReady, signInWithGoogle } = useAuth();
        ...
        {loading ? (
          ...
          <div className="animate-fade-in">
            ...
            {!authEnabled ? (
              ...
              <button
                type="button"
                onClick={() => signInWithGoogle(`/join/${encodeURIComponent(code)}`)}
                disabled={!authReady}
                className="mt-6 flex w-full items-center justify-center gap-3 rounded-xl border border-black/10 bg-cream-card py-3.5 text-[15px] font-medium text-ink shadow-sm disabled:opacity-40"
              >
                <GoogleMark /> Sign in with Google to join
              </button>
            )}
          </div>
        ) : (
